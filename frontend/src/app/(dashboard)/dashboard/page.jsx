...
import { BarChart3, Users, GraduationCap, TrendingUp, Target, Award, Zap } from 'lucide-react';
import {
            ...
            <CardContent>
              {stats?.latestSkills?.length > 0 ? (
                <div className="space-y-3">
                  {stats.latestSkills.sort((a, b) => b.score - a.score).slice(0, 8).map((skill, i) => (
                    <div key={i} className="space-y-1.5">
                      <div className="flex justify-between text-sm">
                        <span className="font-medium text-sm">{skill.skill}</span>
                        <span className="text-muted-foreground font-mono text-xs">{skill.score}%</span>
                      </div>
